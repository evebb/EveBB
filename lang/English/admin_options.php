...
'Board logo label'					=>	'Board logo',
'Board logo help'					=>	'Optionally upload a logo image (PNG, JPEG, GIF or WebP) to show in place of the board-title text. If no logo is set, the text title above is used, and the board title is always used as the image\'s alt text. For a full-width banner, make the image at least about 990&nbsp;px wide (the content width of the bundled styles; ~1980&nbsp;px for sharp display on high-resolution screens) and set the position below to "Full width".',
'Board logo remove'					=>	'Remove the current logo (revert to the text title)',
'Board logo size label'				=>	'Logo size',
'Board logo width'					=>	'Width:',
'Board logo height'					=>	'Height:',
'Board logo size help'				=>	'Optional. A plain number is treated as pixels, or add a CSS unit (e.g. 300px, 8em). For the Left/Centre/Right positions this sizes the whole logo (leave blank for the image\'s natural size). For "Full width, fixed height" the Height box sets the band height (default 200px) and Width is ignored. For "Full width (banner)" both are ignored.',
'Board logo align label'			=>	'Logo position',
'Board logo align left'				=>	'Left',
'Board logo align center'			=>	'Centre',
'Board logo align right'			=>	'Right',
'Board logo align full'				=>	'Full width (banner — whole image)',
'Board logo align cover'			=>	'Full width, fixed height (crop to fill)',
'Board logo align help'				=>	'Where to place the logo. "Full width (banner)" stretches the whole image across the header, keeping its proportions, so the header height follows the image. "Full width, fixed height" keeps a constant header height and crops the image to fill that band (set the height in the Height box above; neither option distorts the image). For a crisp full-width result the image should be at least about 990&nbsp;px wide (the content width of the bundled styles; ~1980&nbsp;px for high-resolution screens). Placement is also exposed to the style sheet (#brdlogo), so a custom style can override it.',
'Logo bad image message'			=>	'The uploaded logo is not a supported image. Please upload a PNG, JPEG, GIF or WebP file.',
'Logo move failed message'			=>	'The uploaded logo could not be saved. Please check that the img directory is writable by PHP.',
...
'Version number help'				=>	'Show eveBB version number in footer.',
...
'Clickable links help'				=>	'When enabled, eveBB will automatically detect any URLs in posts and make them clickable hyperlinks.',
...
'Quick post help'					=>	'When enabled, eveBB will add a quick reply form at the bottom of topics. This way users can post directly from the topic view.',
'WYSIWYG label'						=>	'Visual editor',
'WYSIWYG help'						=>	'When enabled, posts and signatures are written in a visual (WYSIWYG) editor with a formatting toolbar, instead of a plain text box. A "Source" button lets users switch to raw BBCode at any time. When disabled, the plain text box is used.',
'Login throttle label'				=>	'Login throttle',
'Login attempts label'				=>	'Max failed attempts',
'Login lockout label'				=>	'Lockout (minutes)',
'Login throttle help'				=>	'When enabled, an IP address that fails to log in the given number of times is blocked from further attempts for the lockout period. This slows password-guessing without affecting normal use. The block is per IP address, so it cannot be used to lock a member out of their own account.',
...
'Upload directory help'				=>	'The upload directory for avatars (relative to the eveBB root directory). PHP must have write permissions to this directory.',
...
'Report new help'					=>	'If enabled, eveBB will notify users on the mailing list (see above) when a new user registers in the forums.',
'Require profile label'				=>	'Require profile details',
'Require profile help'				=>	'If enabled, new members must supply their real name, date of birth (13+) and country when registering.',
'Captcha label'						=>	'Registration CAPTCHA',
'Captcha help'						=>	'If enabled, visitors must read and type a distorted confirmation code shown as an image before they can register. Helps block automated sign-ups. Requires the GD and OpenSSL PHP extensions.',
